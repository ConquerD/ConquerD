...
**Last reviewed:** 2026-07-09 (backlog.md "Crypto — group key reliability" closed — `SenderKeysGroup::{current_epoch, epoch_key}` now actually consume distributed room key material instead of hardcoding the deterministic key forever, and room-key distribution is driven by a deterministic elected-keyer tie-break (`is_elected_keyer`, smallest `public_id` present) instead of a fixed room "creator", closing the ownerless built-in `default` room gap and covering reconnect-after-drop. See `backlog.md` and `group_key.rs` / `connection_manager/manager.rs`. Test totals refreshed to 629. Previously (2026-07-08): Space Merkle Layer 1 gaps 1–3, 5 (lighter mitigation), and 6 closed — `"members"` invite-policy widening, client UI toggle, periodic Space-root re-broadcast, root-equivocation flagging, and sub-room nesting (Server → Room → Room, arbitrary depth via existing `parent_id`); see `docs/SPACE-MERKLE-DESIGN.md`. Remaining Space Merkle work is legacy-roster-path removal (blocked on a production-verification gate, not actionable), the heavier CT-log root-equivocation redesign, and Layer 2 key hierarchy — all still open. Durable per-feature invariants live in Architecture Notes / Using the Modular Framework / Feature Module Reference above).

### Health summary

ConquerD is in strong shape for a 1.0 privacy-first modular P2P framework: near-zero authored tech debt, dense unit coverage (629 unit tests listed by `cargo test -- --list`: 114 features + 246 supernode + 193 headless client + 76 installer), architecture compliant with the capability-gated, client-only, invite-only model, and solid supply-chain hardening (SHA-pinned actions, version sync, optional signing with graceful fallbacks). Game relay (`game.relay.v1` over WebTransport) is confirmed working end-to-end with native clients. SFU room definitions are client-owned; supernodes host rooms ephemerally only. **Supernode manager** (`rust/conquerd-supernode-manager/`) is production-tested: the acdc three-node cluster (a/b/c) is the live integration-testing target — `build-deploy`, `cluster-sync`, and `exec`-based remote debugging are operational.
