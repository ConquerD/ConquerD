...
- **Three Cargo workspaces**: `rust/` (features, supernode, installer, opus, vpx), `rust/conquerd-client/` (desktop client), and `rust/conquerd-supernode-manager/` (cluster operations). Format/check every workspace you touch. `scripts/ci_local.ps1` covers the two product workspaces; manager changes must also run fmt/check and targeted tests from its own workspace root.
