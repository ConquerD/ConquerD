...
- **Every frame carries its codec**, in fragment-header byte 2 (`FRAGMENT_VERSION` `0x02`), **bound into the frame signature** (`video_frame_signing_bytes`) so a relay flipping it fails verification. An unknown codec byte is refused at parse; fragments of one frame that disagree on codec never combine.
