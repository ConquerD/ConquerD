...
### 5. QA/Testing Agent
Responsibilities:
- Run unit and integration tests and targeted manual checks. Scale as measured on 2026-08-07: **471** listed in the outer workspace (`conquerd-supernode` 236, `conquerd-features` 143 + 3 doc, `conquerd-installer` 80, `conquerd-vpx` 9, `conquerd-opus` 0) and **584** in the headless client workspace, alongside the separate supernode-manager suite. These drift every sprint — always re-derive with `cargo test -- --list` per workspace rather than trusting the figure above.
...
**Last reviewed:** 2026-08-07. Cross-platform video landed over the previous cycle: vendored VP8 (`conquerd-vpx`) on every platform, hybrid codec negotiation, and camera capture on Windows/Linux/macOS. A synced content-audio layer landed beside it: the sound shared with a video rides its own tags (`0x08`/`0x09`) on the video session's clock, with audio-led playout in `media_sync.rs`, and one **Share video** control that chooses the audio at start rather than a second toggle. The voice path is untouched, and the two mixes are independent — muting a peer's voice must not silence what they are sharing (`resolve_mix_gain`).
