    ...
    E: FnMut(StreamError) + Send + 'static,
{
    ...
    // attached, so a speakerphone call feeds its own output straight back
    // into the mic. VoiceCommunication is also the preset the platform
    // expects to pair with an output stream in MODE_IN_COMMUNICATION.
    let builder = builder.set_input_preset(oboe::InputPreset::VoiceCommunication);
    let stream = builder
    ...
    E: FnMut(StreamError) + Send + 'static,
{
    ...
    // AudioManager.setCommunicationDevice() for it, so earpiece / headset
    // selection is impossible no matter what the app asks for. Usage
    // VoiceCommunication + ContentType Speech puts the stream on the call
    // route, which is what makes routing control (and the hardware echo
    // canceller on the matching input stream) work at all.
    let builder = builder
